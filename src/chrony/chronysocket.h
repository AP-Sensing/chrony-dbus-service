    ...
    #define SCK_FLAG_MSG_DESCRIPTOR 2
      ...
      set_unix_sockaddr(struct sockaddr_un *sun, const char *addr)
      {
        ...
        if (len + 1 > sizeof (sun->sun_path)) {
          std::cout << std::format("Unix socket path {} too long", addr) << std::endl;
          return 0;
        }
      ...
      bind_unix_address(int sock_fd, const char *addr, int flags)
      {
        ...
        if (unlink(addr) < 0)
          std::cout << std::format("Could not remove {} : {}", addr, strerror(errno)) << std::endl;

        /* PRV_BindSocket() doesn't support Unix sockets yet */
        if (bind(sock_fd, &saddr.sa, saddr_len) < 0) {
          std::cout << std::format("Could not bind Unix socket to {} : {}", addr, strerror(errno)) << std::endl;
          return 0;
        }

        /* Allow access to everyone with access to the directory if requested */
        if (flags & SCK_FLAG_ALL_PERMISSIONS && chmod(addr, 0666) < 0) {
          std::cout << std::format("Could not change permissions of {} : {}", addr, strerror(errno)) << std::endl;
          return 0;
        }
