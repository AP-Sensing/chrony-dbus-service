#ifndef CHRONYPKTLENGTH_H
#define CHRONYPKTLENGTH_H
...
#define PADDING_LENGTH_(request_length, reply_length) (uint16_t)((request_length) < (reply_length) ? (reply_length) - (request_length) : 0)

#define PADDING_LENGTH(request_data, reply_data) PADDING_LENGTH_(offsetof(CMD_Request, request_data), offsetof(CMD_Reply, reply_data))

#define REQ_LENGTH_ENTRY(request_data_field, reply_data_field) \
    {offsetof(CMD_Request, data.request_data_field.EOR), PADDING_LENGTH(data.request_data_field.EOR, data.reply_data_field.EOR)}

#define RPY_LENGTH_ENTRY(reply_data_field) offsetof(CMD_Reply, data.reply_data_field.EOR)
...
static const struct request_length request_lengths[] = {
    ...
    REQ_LENGTH_ENTRY(ntp_source_name, ntp_source_name),                   /* NTP_SOURCE_NAME */
    REQ_LENGTH_ENTRY(null, null),                                         /* RESET_SOURCES */
    REQ_LENGTH_ENTRY(auth_data, auth_data),                               /* AUTH_DATA */
    REQ_LENGTH_ENTRY(client_accesses_by_index, client_accesses_by_index), /* CLIENT_ACCESSES_BY_INDEX3 */
...
int PKL_CommandPaddingLength(CMD_Request *r)
{
    ...
    if (r->version < PROTO_VERSION_PADDING) return 0;

    type = ntohs(r->command);

    if (type >= N_REQUEST_TYPES) return 0;
...
int PKL_CommandLength(CMD_Request *r)
{
    ...
    assert(sizeof(::chrony::pktlength::request_lengths) / sizeof(::chrony::pktlength::request_lengths[0]) == N_REQUEST_TYPES);

    type = ntohs(r->command);
    if (type >= N_REQUEST_TYPES) return 0;

    command_length = ::chrony::pktlength::request_lengths[type].command;
    if (!command_length) return 0;

    return command_length + ::chrony::pktlength::PKL_CommandPaddingLength(r);
}
...
int PKL_ReplyLength(CMD_Reply *r)
{
    ...
    if (type < 1 || type >= N_REPLY_TYPES) return 0;

    return reply_lengths[type];
}
}  // namespace pktlength
}  // namespace chrony
#endif  // CHRONYPKTLENGTH_H
