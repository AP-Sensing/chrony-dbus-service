...
#define INACTIVE_AUTHKEY 0
  namespace client
  {
      ...
      static int open_unix_socket2(const std::string &af_unix_address, const std::string &local_addr="")
      {
        ...
        {
          std::cerr << "Error binding local socket" << std::endl;
        }
      ...
      static int
      open_socket(struct chrony::socket::Address *addr)
      {
        char *dir, *local_addr;
      ...
      open_io(void)
      {
        ...
        for (chrony::socket::Address address : server_addresses) {
          ...
          {
            std::cout << "open_io(): open_socket success!" << std::endl;
            return 1;
          }
          else
          {
            std::cerr << "open_io(): open_socket failed!" << std::endl;
          }
      ...
      {
        std::cout << "submit_request(): enter" << std::endl;
        int quit=0;
        int max_retries=2;
        ...
        do {
          ...
          if (new_attempt) {
            ...
            std::cout << "submit_request(): n_attempts/max_retries: " << n_attempts << "/" << max_retries << std::endl;
            ...
            request->sequence = htons(randomGenerator()); /// @todo don't know if it is a security risk to use pseudorandom numbers here instead of /dev/urandom
            ...
            std::cout << "submit_request(): request request->sequence: " << request->sequence << std::endl;
            std::cout << "submit_request(): request proto_version: " << proto_version << std::endl;
            std::cout << "submit_request(): request command_length: " << command_length << std::endl;
            std::cout << "submit_request(): request padding_length: " << padding_length << std::endl;
            ...
            if (sock_fd < 0) {
              std::cerr << "submit_request(): No socket to send request" << std::endl;
              return 0;
            }

            if (command_length < 0) {
              std::cerr << "Invalid length " << command_length << std::endl;
              return -1;
            }
            ...
            if (r < 0) {
              std::cerr << "Could not send data fd=" << sock_fd << " len=" << command_length << " : " << strerror(errno) << std::endl;
              return r;
            }

            std::cout << "Sent data fd=" << sock_fd << " len=" << r << std::endl;
          }

          ::chrony::util::UTI_TimevalToTimespec(&tv, &ts_now);

          /* Check if the clock wasn't stepped back */
          if (::chrony::util::UTI_CompareTimespecs(&ts_now, &ts_start) < 0)
            ts_start = ts_now;

          timeout = initial_timeout / 1000.0 * (1U << (n_attempts - 1)) - ::chrony::util::UTI_DiffTimespecsToDouble(&ts_now, &ts_start);
          std::cerr << "submit_request(): Timeout " << timeout << " seconds" << std::endl;
          ...
          if (select_status < 0) {
            ...
            std::cerr << "submit_request(): select_status == 0!" << std::endl;
            ...
            if (recv_status < 0) {
              std::cerr << "submit_request(): recv_status < 0!" << std::endl;
              new_attempt = 1;
            } else {
              ...
              if (static_cast<unsigned long int>(read_length) < offsetof(CMD_Reply, data) ||
                ...
                std::cerr << "submit_request(): Invalid reply" << std::endl;
                continue;
              }
    ...
    #error unknown compatibility with PROTO_VERSION - 1
              ...
              if (read_length < ::chrony::pktlength::PKL_ReplyLength(reply)) {
                std::cerr << "submit_request(): Reply too short" << std::endl;
                new_attempt = 1;
                continue;
              }

              /* Good packet received, print out results */
              std::cerr << "Reply cmd=" << ntohs(reply->command) << " reply=" << ntohs(reply->command) << " stat=" << ntohs(reply->status) << std::endl;
              break;
            }
          }
        } while (1);
      ...
      request_reply(CMD_Request *request, CMD_Reply *reply, int requested_reply, int verbose)
      {
        ...
        while (!submit_request(request, reply)) {
          std::cout << "request_reply(): trying to submit request" << std::endl;
        ...
        std::cout << "request_reply(): request status: " << status << std::endl;
      ...
      static std::tuple<std::vector<ChronySourceData>, bool>
      process_cmd_sources()
      {
        std::vector<ChronySourceData> retVal;
        std::cout << "process_cmd_sources(): enter" << std::endl;
        CMD_Request request;
        CMD_Reply reply;
        IPAddr ip_addr;
        ...
        std::cout << "process_cmd_sources(): n_sources: "<< n_sources << std::endl;
        ...
        for (i = 0; i < n_sources; i++) {
          ...
          std::cout << "process_cmd_sources(): source i: " << i << " ip_addr.addr.in4: " << chrony::util::UTI_IPToString(&ip_addr) << std::endl;
      ...
      static int
      process_cmd_add_source(const AddServersData &data)
      {
          int result = 0;
          CMD_Request request;
          CMD_Reply reply;
      ...
      static int
      process_cmd_delete(const std::string &serverAddress)
      {
        int result = 0;
        IPAddr address;
        CMD_Request request;
        ...
        if(chrony::util::UTI_StringToIP(serverAddress.c_str(), &address)!=0)
        {
          ...
          {
            std::cerr << "Could not parse serverAddress for process_cmd_delete" << std::endl;
            result = 0;
            return result;
          }
        }
      ...
      static std::tuple<std::vector<std::string>, bool>
      ...
      static int
      process_cmd_settime(const std::string& newTimeString)
      {
        struct timespec ts;
        time_t new_time;
        CMD_Request request;
        ...
        std::cout << "Setting time to: " << timeTemp.tm_hour << ":" << timeTemp.tm_min << ":" << timeTemp.tm_sec << std::endl;
